...
interface EliminarPagoTerceroModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export default function EliminarPagoTerceroModal({
  isOpen,
  onClose,
  payment,
  onConfirmDelete
}: EliminarPagoTerceroModalProps) {
  ...
  const isPaid = payment.estatusPago === 'Pagado';

  return (
    <div id="eliminar-pago-tercero-modal-container" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div id="eliminar-pago-tercero-modal-card" className="bg-white dark:bg-[#051A14] w-full max-w-md rounded-lg shadow-2xl border border-cranberry/30 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-enchanted-green/10 dark:border-light-ivory/10 flex items-center justify-between">
          <h3 className="font-serif text-base font-semibold text-cranberry flex items-center space-x-2">
            ...
            <span>Eliminar Pago a Tercero</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
            ...
            <div className="flex items-start space-x-3 bg-cranberry/5 p-3.5 rounded border border-cranberry/25">
              ...
              <div className="space-y-2 w-full">
                ...
                <p className="text-sm text-[#082019] dark:text-light-ivory font-semibold">
                  Concepto: {payment.concepto} ({formatCurrency(payment.montoADepositar)})
                </p>
                
                {isPaid ? (
                  <p className="text-xs text-[#082019] dark:text-light-ivory/95 leading-relaxed font-semibold">
                    Este pago ya fue marcado como dispersado al tercero. Eliminarlo solo borra el registro interno — no revierte ninguna transferencia de dinero real ya realizada. ¿Continuar?
                  </p>
                ) : (
                  <p className="text-xs text-[#082019] dark:text-light-ivory/95 leading-relaxed font-semibold">
                    ¿Eliminar este registro de Pago a Tercero pendiente? No se puede deshacer.
                  </p>
                )}
              </div>
            </div>
